...
public:
	...
	*/
	...
	{
		cout << "usa isSame press a button to continue... ";
		cin.get();
		return isSame("xx", "yy");
	...
	static vector<string> tokenize(const char* s,const char* delim)
	{
	//	cout << "Ojo aquí usa tokenize press a button to continue... ";
//		cin.get();
		...
		string str1(s);
		string &str = str1;
		// WE - I included the code of the method tokenize because it is sending 
		// as a parameter the address of the char* s string& str and it is not allowed for linux basic code
//		static vector<string> tokenize(string& str, char* delim) {
			vector<string> tokens;
			string deli(delim);
			// Skip delimiters at beginning.
