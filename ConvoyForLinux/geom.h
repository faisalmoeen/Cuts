...
public:
	...
	Point2D operator=(const Point& p) { return Point2D(x = p.x, y = p.y); }
	Point2D operator+(const Point2D& p) { return Point2D(x + p.x, y + p.y); }
	Point2D operator-(const Point2D& p) { return Point2D(x - p.x, y - p.y); }
	Point2D operator+(const Point& p) { return Point2D(x + p.x, y + p.y); }
	Point2D operator-(const Point& p) { return Point2D(x - p.x, y - p.y); }
	Point2D operator*(const double c) { return Point2D(x*c, y*c); }
	Point2D operator/(const double c) { return Point2D(x / c, y / c); }
};
...
public:
	...
	void min_dist(MBR &m, double* rst) { min_dist(&m, rst); }
	void min_dist(MBR *m, double* rst) { double d;min_dist2(m, &d);*rst = sqrt(d); }
	void min_dist2(MBR &m, double* rst) { min_dist2(&m, rst); }
	void min_dist2(MBR* m, double* rst)
	{
		double xd, yd;
		if (maxx < m->minx)
			xd = m->minx - maxx;
	...
	void max_dist2(MBR* m, double* rst)
	{
